...
import { CalendarDays, Users, Upload, LayoutGrid, KanbanSquare, FileBarChart, CheckCircle2, Zap } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.section
        ...
        className="premium-hero p-0 overflow-hidden"
      >
        <div className="relative h-72 overflow-hidden sm:h-80">
          {project.coverImage ? (
            ...
            <>
              <div className="h-full w-full bg-gradient-to-br from-[#090D1A] via-blue-950/20 to-[#090D1A] flex items-center justify-center text-white">
          ...
          {isAdmin && (
            <label
              ...
              className="absolute right-4 top-4 inline-flex cursor-pointer items-center gap-2 rounded-xl bg-blue-600/80 hover:bg-blue-600 px-3 py-2 text-xs font-semibold text-white backdrop-blur transition-all"
            >
          ...
          <div className="absolute inset-x-0 bottom-0 px-6 pb-6 text-white sm:px-10">
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-blue-400">Project overview</p>
            <h1 className="mt-3 text-4xl font-extrabold tracking-tight leading-none text-white">{project.title}</h1>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3 text-sm text-slate-200">
              <span className="inline-flex items-center gap-2 rounded-3xl bg-black/40 border border-white/5 px-3 py-1.5 backdrop-blur-sm">
      ...
      {project.teamMembers && project.teamMembers.length > 0 && (
        ...
        >
          <div>
            <h2 className="text-2xl font-bold text-white">Assigned Team Members</h2>
            <p className="mt-2 text-sm text-slate-400">{project.teamMembers.length} member{project.teamMembers.length !== 1 ? 's' : ''} assigned to this project</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {project.teamMembers.map((member) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="premium-card bg-gradient-to-b from-[#131B2E] to-[#0E1424] p-5"
              >
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 to-blue-550 text-white font-bold">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 truncate mt-1">{member.email}</p>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      >
        <div>
          <h2 className="text-2xl font-bold text-white">Workspace Hub</h2>
          <p className="mt-2 text-sm text-slate-400">Navigate between project tools and collaboration spaces</p>
        </div>
              ...
              >
                <Link to={tab.route}>
                  <motion.div
                    ...
                    className={`h-full premium-card p-5 cursor-pointer ${
                      isActive
                        ? "bg-gradient-to-br from-blue-600 to-blue-500 text-white shadow-lg shadow-blue-500/25 border-none!"
                        : ""
                    }`}
                  >
                    <div className="flex items-start gap-3">
